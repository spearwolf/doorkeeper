...
export default class TokenStoreRedis {
  ...
  async verifyAndUpdateLastLoginTime(uid, time) {
    console.log(`REDIS VERIFY AND UPDATE LAST LOGIN:`, uid, time);
    const key = this.#constructLastLoginTimeKey(uid);
    const ok = await this.client.SET(key, time, "XX", "EX", this.lastLoginExpire);
    console.log(`REDIS RESPONSE: "${ok}"`);
    if (ok !== "OK") {
      throw "no last login";
    }
  }
